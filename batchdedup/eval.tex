...
\begin{figure}[ht]
  \centering
	\begin{tikzpicture}
		...
		%title={Network Buffer Size},
		xlabel={Rounds},
		ylabel={Total Jobspan (m)},
		%extra y ticks={4.5,5.5,6.5} %to show extra ticks
                %legend pos=north west
		]
                \addplot table[x=Rounds,y expr=(\thisrow{DBP2jobspan})/60] {figures/packing_comparison.txt};
                \addplot table[x=Rounds,y expr=(\thisrow{BPLjobspan})/60] {figures/packing_comparison.txt};
                \legend{DBP2, BPL};
		\end{axis}
	\end{tikzpicture}
