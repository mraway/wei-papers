...
because of the workload in stage 1a is imbalanced (see Fig.~\ref{fig:hist-1}),
causing machines with small VMs to wait while CoW protection is turned on.
Thus we must minimize the duration that a given VM is undergoing CoW protection.r
...
A representative histogram for one of the clusters is shown in Fig.~\ref{fig:hist-1}.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
      ...
      \end{axis}
  \end{tikzpicture}
  \caption{Histogram of average data sizes for a real cluster. TODO:reformat graph, I just wanted to get the data into the paper}
  \label{fig:hist-1}
\end{figure}
