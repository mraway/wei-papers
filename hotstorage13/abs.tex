...
hosting  reliability; howver, it  takes significant 
memory and CPU resource to perform data duplication in order to remove
excessive redudancy among image snapsots.
This paper presents a low-cost clustered deduplication soluton scalable for handling a large number 
of virtual machines.  The key idea is to seperate duplicate detection from 
the actual storage data backup instead of using inline deduplication.
Global inddex and  detection requests are partitioned 
among machines using  fingerprint values for parallel detection. 
Each machine conducts duplicate detection partiton by partition indepdently
