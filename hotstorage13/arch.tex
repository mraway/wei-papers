...
\caption{Processing flow in Phase 1 (VM scan/request accumulation) and Phase  2 (fingerprint comparison and duplicate summary output)
...
%\item
The second phase is to load disk data and perform duplicate detection at each machine for  all accumulated 
...
reference. 
%Phase 3 is that every VM gathers its z confirmed deletion requests for its content and sends them to the backup storage.
