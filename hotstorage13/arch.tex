\section{ Key Ideas and Design Architecture}
\label{sect:arch}
...
The data flow of our partition-based  dupliate detectionis depicted in
...
remapped from fingerprinter-based distribution  to VM-based distribution. In this way, duplicate information 
for segments of each VM is collected in one place, which will be used in next step. 
In the sending side, each machine scans idenitified duplicates for one target machine,
and then scans for another machine. In this way, it sends dummary summary to the corresponding machines one by one
...
\item The third step is to read  nonduplicate blocks from each VM and output to the final backup 
storage. In this process, if a segment is not dirty, we only need to output the segment meta data 
...
{\bf Snapshot deleteion.} Each virtual machine will keep at most $x$ snapshots of VM images and thus
