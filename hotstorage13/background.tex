...
Backup systems have been developed to use content hash (fingerprints) to identify duplicate
...
by using an in-memory Bloom filter and prefetching groups of chunk IDs that are likely to be
...
Additional inline deduplication techniques are studied in ~\cite{sparseindex09,Guo2011,idedup}. All of the above approaches have focused on
such inline duplicate detection in which  the duplicate detection  speed of an individual block  is critical.
...
important than optimizing individual content block backup requests.
