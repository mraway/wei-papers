...
Noted that this paper optimizes the cost of backup when such a service runs on the existing
cluster resource for the cloud. Another option is to attach  a separate backup storage system with deduplication
support  the cloud cluster and every machine can periodically transfer snapshot data to
...
not be fully utilized. This paper seeks for on a low-cost architecture option.
