...
We have implemented a prototype of the parititon-based deduplication scheme on a Linux cluster
based on Alibaba's Zene cloud platform.  Objectives of our experimental evaluation are:
1) Analyze the processing time and througput of backup for a large cluster of virtual machines.
...
At Alibaba's Aliyun our target is to backup cluster up to 1000 nodes with 25 VMs on each machine.
...
In this dataset, there are 10 snapshots per each VM user and the total amount of space investigated is 23.5 terabytes.
...
We have also used a combination of level 1 duplicate detection using segement dirty bits and
a bloom filter and prefetch cache  after level 1 basd on~\cite{Zhu}. 
The bloomer filter settings  resutls a 1:10 ratio of index reduction for in-memory search before visiting
the disk for the full index with 2\% false positives. The prefetching cache hit ratio is set to be 98\% based
a number in ~\cite{Zhu}. Our actual cache hit ratio tested is close 90\%.
\comments{
...
\begin{verbatim}
q		50	100	250	275	300	450	500	750	1500	2000
...
Seek Option 6.388888889	12.77777778	31.94444444	35.13888889	38.33333333	57.5	63.88888889	95.83333333	191.6666667	255.5555556
...
the number of partitons per machine varis from 50 to 2000.
This is the overall time for a 100-node cluster and the throughput per machine reported  is the total 
amount of virtual machine images divided by the processing time. The processng time includes the first scanning time
of dirty VM segments to generate block figureprints and the second scann time for real backup.
