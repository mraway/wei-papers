...
We have implemented and evaluated a prototype of the parititon-based deduplication scheme on a Linux cluster
...
We have performed a trace-driven study using a dataset containing 10 snapsots of 35 VMs.
That is part of a 1323 VM dataset  collected from 100 Aliyun's cloud nodes 
in our earier work with  Alibaba Aliyun~\cite{WeiZhang}.
...
the number of partitons per machine ($q$) varies from 50 to 2000.
...
The processng time includes the first scanning time
of dirty VM segments to generate block figureprints and the second scan time for real backup.
...
The overall processing time does not have a signficant reduction as we increase memory usage  20MB to 200MB.
As Row 2 shows, the time is mainly dominated by  the IO costs of two scans of VMs.
Thus we conlude that using about 30MB is sufficient to have VM backup completed in parallel within about 3.11 hours.
...
dirtybit segement  detection and  the data domain method~\cite{Zhu}. 
...
We measure its  total time including one scan of VM dirty segemnts in about 1.38 hours.
...
compeuting with other cloud service while memory cost of our setting is  insignifcant.
