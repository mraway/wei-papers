...
and it requires the global reference of deldted chunks to be identified before  they can be safely removed.
...
to efficiently identify unused chunks in $O(n)$ time, with $n$ being the logical number of non-PDS chunks 
to be deleted from a VM snapshot store.  The algorithm contains three aspects.
