...
%uses the logical a bloom filter to check if a data are still referenced by living snapshots. 
{\bf Discussion}
For one VM's snapshots, let $D_{leak}$ be the amount of storage leakage, 
...
$N_1$ be the number of runs of approximate deletion phase-1, then we have:
\begin{equation}
D_{leak} = N_1 * P_b * D_{del}
\end{equation}

We let approxmite deletion phase-2 be triggered  
when $D_{leak}/D_{del}$ is accmulated to exceed certain threshold $t$:
\begin{equation}
\frac{D_{leak}}{D_{del}} = N_1 * P_1 > t \Rightarrow N_{1} > \frac{t}{P_1}
\end{equation}
