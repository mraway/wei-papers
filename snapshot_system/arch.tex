...
we compute a bloom-filter as the summary of reference pointers for all non-PDS chunks used 
in this snapshot.
...
Consider each reference pointer consumes 8 bytes plus 1 byte as mark field, a VM that has 40GB backup data will
need 90MB of memory to construct such a table. Also, scanning all snapshots metadata is many times slower comparing
...
%{\bf Discussion}
We now give a model to estimate the storage leakage and how often leak repair needs to be conducted.
Assuming a VM always maintain $B$ snapshots in the backup, and it creates and deletes one snapshot
everyday. Let $c$ be the total number of chunks brought by the initial backup, $c^'$ be the average
number of chunks brought by a consequancial backup, then the total number of chunks in use $C$ is:
\[
C = c + (B-1)c^'
\]

Let $m$ be the bloom filter size in bits, $k$ be the number of hash functions used by bloom filter,
then bloom filter's false-positive rate $\epsilon$ is:
\[
\epsilon = (1-e^{\frac{kC}{m}})^k
\]

For each snapshot deletion, the amount of chunks need to be deleted is nearly identical to the number of
newly add chunks $c^'$. However, some of chunks among them are not detected as unused in our approximation
algorithm, thus forms the storage leakage. Let $R$ be the total number of runs of approximate deletion since
last repair, the total leakage $L$ can be estimated as:
\[
L = Rc^'\epsilon
\]

When leakage ratio $L/C$ exceeds a pre-defined threshold $\tau$, we need to execute a repare:
\[
\frac{L}{C} = \frac{Rc^'\epsilon}{c+(B-1)c^'} > \tau \rightarrow R > \frac{\tau}{\epsilon}\frac{c + (B-1)c^'}{c^'}
\]
For example, when $\epsilon = 0.01$ and $\tau=0.1$, when the VM keeps 10 snapshots and each snapshot contains
about 5\% of new data, then
we would expect leak repair be triggered once for 
