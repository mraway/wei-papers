...
which  occurs frequently when snapshots expire regularly and leads a cost concern. 
...
\item{\bf Management of file system blocks.}
The file block size in a distributed file system such as  Hadoop and GFS is uniform and large (e.g.  64MB),
 a data chunk in a deduplication service is of a nonuniform size with 4KB or 8KB on average.
...
We call the classical deduplication approach as   VM-oblivious (VO)
...
cross-VM  signature comparison can deliver additional reduction~\cite{Guo2011,Dong,ExtremeBining}.
...
While this approach is VM-oblivious, we can have adopted some of ideas to build
bins in a VM to faciliate VM-specific local search. 
