...
\begin{figure}[ht]
  \centering
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
  \caption{Measured Average number of VMs sharing a 64MB FSB with global dedup (VO), and in the PDS for VC.}
  \label{fig-fsb-links}
\end{figure}
...
(Figure~\ref{fig-fsb-links}) in our dataset. For the complete dataset, we
also found the average number of links to PDS chunks, which was 11 using a 5\%
...
data reliability for the VC model. As you can see from the graph, even small
changes in the number of links to VO blocks has a significant impact on
reliability, while even a 100 times increase in the VC block link counts only
  ...
  %\includegraphics[scale=.45,natwidth=511,natheight=276]{vo_links}
	\begin{tikzpicture}
		...
		extra y ticks={99.9}, %if important numbers are missing
            ...
            \legend{$VO\,10$ (optimistic),$VO\,5$ (measured),$VC\,1000$ (extreme),$VC\,11$ (measured)}
		\end{axis}
	\end{tikzpicture}
