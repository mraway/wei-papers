  ...
  \centering
	\begin{tikzpicture}
		...
		legend pos=north west
                ...
                \addplot table[x=VMs,y=No] {figures/vo_vm_links.txt};
		\addplot table[x=VMs,y=N1] {figures/vc_vm_links.txt};
                \addplot+[red,densely dashed] table[x=VMs,y=N2] {figures/vc_vm_links.txt};
                \addplot+[red,dotted,mark=none] table[x=VMs,y expr=\thisrow{N1}+\thisrow{N2}] {figures/vc_vm_links.txt};
...
tolerance), it should do so slowly. for VC we use 1250, because that is what we
  ...
  %\includegraphics[scale=.45,natwidth=511,natheight=276]{vo_links}
	\begin{tikzpicture}
		...
		extra y ticks={99.9}, %if important numbers are missing
                ...
                \addlegendentry{$VC\,2500$ (worst-case)}
		...
		\end{axis}
	\end{tikzpicture}
  \caption{Availability of VM backups as nodes fail for VO and VC models for
  varying degrees of block sharing (i.e. average number of VMs that use a
  block). Non-PDS replication fixed at 3 and PDS replication increased to 4.
  Graph shown for 100 machine cluster.
  }
...
\begin{figure}[ht]
  \centering
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
  \caption{Availability of VM backups as nodes fail in the VC model for different PDS Replication factors (Non-PDS replication fixed at 3, and average PDS block links set to 1250)}
