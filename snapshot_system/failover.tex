...
\begin{figure}
    ...
    {
        \includegraphics[width=3in]{images/share_model_vc.pdf}
        \label{fig:share_vc}
    }
    \\
    \subfigure[Sharing of data under VM-centric dedup model]
    {
        \includegraphics[width=3in]{images/share_model_vo.pdf}
        \label{fig:share_vo}
    }
...
is insignificant.
Now we compare the fault resilience of VC and VO.
...
Each non-CDS file system block is associated with one VM while CDS file system blocks are
shared among VMs. 
\[
...
Figure~\ref{fig-vo-links} shows the average number of VMs shared by 
a block in the global index as more VMs are added. 
...
The availability of is a VM is the likelyhood that
...
should continue to increase at a slower rate. 
For the VC model, we have to consider both CDS chunks and 
...
The figure shows that even as the number of VMs sharing a CDS file block 
increase by 100 times, the availability only decreases
slightly.
The key factor placed is that  $N_1 +N_2 << N_o$.
The VM-centric approach packs  data blocks under one VM as much as possible
and performs deduplication mainly within the same VM. The sharing part in CDS is replicated
with a higher degree, which also signficantly increses the availability even a CDS file blcok
