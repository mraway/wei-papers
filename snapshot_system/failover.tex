...
The first 15 VMs added are all windows (though the last one has a much larger
hard drive which explains the drop after 14), which is why the number of block
links goes up quickly for the first half of the graph. You can see in the plot
...
per block (Figure~\ref{vo-links}) and number of links per chunk (Figure~\ref{fig-vo-fsb-links})
in our dataset. For the complete dataset, we also found the average number of links to CDS chunks, which
was 11 using a 5\% CDS. From Figure~\ref{fig-vo-fsb-links} you can see that as you add VMs, the average
