...
a block in the global index as more VMs are added in VO. 
...
per block (Figure~\ref{vo-links}) and number of links per chunk (Figure~\ref{vo-fsb-links})
in our dataset. For the complete dataset, we also found the average number of links to CDS chunks, which
was 11 using a 5\% CDS. From Figure~\ref{vo-fsb-links} you can see that as you add VMs, the average
...
slightly.
The key factor placed is that  $N_1 +N_2 << N_o$, caused by the fact that
the VM-centric approach localizes deduplication and
packs  data blocks for one VM as much as possible.
The extra replicaton for CDS blocks 
also signficantly increses the snapshot availability even a CDS file block
