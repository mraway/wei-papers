...
\begin{figure}[ht]
  \centering
	\begin{tikzpicture}
		...
		ylabel={\small Avg. Num. VMs sharing FSB},
		%extra y ticks={4.5,5.5,6.5} %to show extra ticks
		legend pos=north west
		]
                \addplot table[x=VMs,y=FSBLinks] {figures/vo_fsb_links.txt};
		\addplot table[x=VMs,y=cdslinks] {figures/cds_links.txt};
                \legend{VO, PDS in VC};
		\end{axis}
	\end{tikzpicture}
...
\begin{figure}[ht]
  \centering
	\begin{tikzpicture}
		...
		%title={VO VM links},
		xlabel={Number of VMs},
		ylabel={\small Avg. Num. FSBs used by VM},
                ymin=0,
                mark options=solid,
		%extra y ticks={4.5,5.5,6.5} %to show extra ticks
		legend pos=north west
		...
		\addplot[color=red,densely dashed,mark=triangle*,mark size=2.7] table[x=VMs,y=VMCDSLinks] {figures/vc_vm_links.txt};
                \legend{$N_0$, $N_1$, $N_2$};
		\end{axis}
	\end{tikzpicture}
...
\begin{table}
    ...
    \hline
    \end{tabular}
    \caption{Availability in a 100 machine cluster of a single FSB in the global data store with different relplication factors}
...
\begin{figure}[ht]
  \centering
    \begin{tikzpicture}
            ...
            ylabel={\small Availability of Single FSB (\%)},
  ...
  %\includegraphics[scale=.45,natwidth=511,natheight=276]{vo_links}
	\begin{tikzpicture}
		...
		extra y ticks={99.9}, %if important numbers are missing
                ...
                \addplot table[x=NodesFailed,y=VC1]{figures/vm_failure_2.txt};
                \addlegendentry{$VC\,20$ (measured)}
                \addplot table[x=NodesFailed,y=VC2]{figures/vm_failure_2.txt};
                \addlegendentry{$VC\,1000$ (extreme)}
                %\addplot table[x=NodesFailed,y=VC3]{figures/vm_failure_2.txt};
                %\addplot table[x=NodesFailed,y=VC4]{figures/vm_failure_2.txt};
                \addplot table[x=NodesFailed,y=VO1]{figures/vm_failure_2.txt};
                \addlegendentry{$VO\,5$ (measured)}
                \addplot table[x=NodesFailed,y=VO2]{figures/vm_failure_2.txt};
                \addlegendentry{$VO\,10$ (optimistic)}
                %\addplot table[x=NodesFailed,y=VO3]{figures/vm_failure_2.txt};
                %\addplot table[x=NodesFailed,y=VO4]{figures/vm_failure_2.txt};
		\end{axis}
	\end{tikzpicture}
