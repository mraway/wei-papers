...
%The dirty bit tracking is valuable because it immediately cuts down the number of blocks
%which must be processed, and reduces memory, network, and cpu resource requirements.
...
In Figure~\ref{fig:share} we build a bipartite graph representing the
...
non-PDS data in our system to acheive the desired $r_c/r$ ratio (though both can be stored on the same DFS).
...
An association edge is  drawn  from an FSB to a VM 
