...
To control the time cost of search, we set a limit on the the number of  similar segments to be  fetched. 
% Then,
...
Noted that  if  the backup for multiple VMs were conducted concurrently, there would be more
...
A(r)^{N_1} * A(r_c)^{N_2}
\end{equation}
Figure~\ref{fig:fsb-availability} illustrates the $A(r)$ values with
...
\begin{table}
    ...
    \begin{tabular}{|l|l|l|l|}
        \hline
        \multirow{2}{*}{Failures}   & \multicolumn{3}{c|}{FSB Availability} \\
                                    %\cline{2-4}
                                    & $R_c=3$ & $R_c=6$ & $R_c=9$ \\
        \hline
        3 & 99.999 & 100 & 100\\
        5 & 99.994 & 100 & 100\\
        10 & 99.925 & 99.999 & 99.999\\
        20 & 99.295 & 99.997 & 99.999 \\
        \hline
    \end{tabular}
    \caption{Availability of individual 64MB Filesystem blocks (FSBs) as storage
    nodes fail in a 100 node cluster.}
