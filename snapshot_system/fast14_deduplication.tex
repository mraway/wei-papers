...
dirty segments' chunk fingerprints within  {\em similar} segments from its parent snapshot. 
...
and discuss this in detail in Section~\ref{sect:architecture}.
...
for  popular data chunks because sampling requires the use of prefetching to be effective.
For the data sets we have tested, the spatial locality is limited among popular
data chunks and  on average the number of consecutive data chunks is 7 among popular chunks,
...
$N_1<N_o$. 
Figure~\ref{fig:fsb-links} shows the number of VMs sharing each file block,
which shows how many VMs the failure of a single block can affect. We only show
links to PDS blocks for VC because Non-PDS blocks in VC are always pointed to
by exactly one VM.
We can also observe that $N_1 +N_2 < N_o$ in Figure~\ref{fig:vm-links}. 
