...
While the predicted value remains flat, PDS  coverage actually increases as more VMs are involved.
increases. This is because actual $\alpha$ value increases with the data size.
%Then  PDS coverage PDS increases as more VMs are involved.

\subsection{ Impact Fault Isolation}
...
the probability of losing a snapshot file block of a VM by
...
every PDS file block is shared by all VMs in the VC approach, which leads a large $N_2$ value. 
