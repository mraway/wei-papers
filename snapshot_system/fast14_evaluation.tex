...
by increasing replication for those blocks we minimize the effect on snapshot availability.
...
\begin{table}
  \centering
    ...
    \pgfplotstabletypeset[
        ...
        columns/VO100/.style={
            column name={
                \multicolumn{4}{c|}{Snapshot Availability(\%)}\\&
...
``Data read/write'' includes disk reading and writing, and 
updating of the metadata.
`` Network cost" includes the cost of transferring snapshot and meta data from one machine to another during backup.
``Index access/lookup'' includes the time for 
latency to visit
