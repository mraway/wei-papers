...
``Read'' column is controlled as 50MB/s bandwidth usage with I/O throttling so that other cloud services are not impacted a lot.
...
The dashed line is the predicted leakage using Formula~\ref{eq:leakrepair} from Section~\ref{sect:delete}
given $\Delta u/u=0.02$,
while the top solid line represents the actual leakage measured during the experiment 
with the same $\Delta u/u=0.02$ setting for summary vectors.
After 9 snapshot deletions, the actual leakage reaches 0.0018\% and this means that
