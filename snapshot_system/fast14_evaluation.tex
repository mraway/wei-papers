...
\begin{table}
    ...
    \begin{tabular}{|l|l|l|l|l|l|l|}
        ...
        \hline
    \end{tabular}
    \caption{Availability of VM snapshots for VO and VC. VC(WC) stands for the case that a 
PDS file block is shared by all VMs.}
...
The total storage  for all VM snapshots in each physical machine takes about 3.065TB on aveage when
...
Table ~\ref{tab:resource_usage} gives the CPU, memory and disk read/write  workload when
running 1 or multiple  backing tasks at the same time on each physical machine. 
Disk read is controled as 50MB/s bandwidth usage so that other cloud services are not impacted a lot.
...
more tasks in parallel at each machine. But adding more bakcup concurrency does not shorten the overall time
