...
\hline
$r$, $r_c$ & replication degree of non-PDS, and PDS file blocks in VC\\
\hline
...
\hline
$N_1$, $N_2$ & the average number  of non-PDS and PDS file blocks in a VM for VC\\
\hline
$N_o$ & the average number  of file blocks  in a VM for VO\\
\hline
...
Each non-PDS block is associated with one VM
 and  we denote that PDS blocks are
shared by an average of $V_c$ VMs. 
Then, 
\[
n* N_1   \approx \mbox{ Number of non-PDS file system blocks} 
\]
...
\[
n *N_o   =  V_o *\mbox{ Number of file system blocks} 
\]

Since the number of PDS file system blocks is small, 
and the deduplication efficiency of VC is very close to that of VO,
Section~\ref{sect:evaluation} shows that
the number of non-PDS file blocks in VC is close to the number of file blocks in VO.
Then
\[
...
\begin{figure}[htbp]
  \centering
	\begin{tikzpicture}
		...
		ylabel={Avg. no file system blocks  used by VM},
                xmin=0,
                ymin=0,
                xmax=106,
...
Given $d$ failed physical machines and $r$ replicas fr each block,
...
\begin{itemize}
\item
When $r \le d<r_c$,  there is no PDS data loss and we need to look at the non-PDS data loss. 
