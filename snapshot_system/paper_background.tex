...
are studied in ~\cite{extreme_binning09,Guo2011,sparseindex09,Guo2011,Srinivasan2012,WeiZhangIEEE}.  
...
For example, in the experiment discussed in Section~\ref{sect:dedup}, 
cross-machine fingerprint comparison using Bloom filter and approximated routing~\cite{bottleneck08,Dong2011} 
still needs several gigabytes of memory per machine, which impacts other cloud services sharing the same
computing resource while our objective is to have an approximation scheme 
which uses about 100 or few hundred megabytes of memory.
...
the file system block  size is often chosen to be large. On the other hand, 
