...
We seperate the deduplication within a VM and cross VMs.
This is because global deduplication that detects the apperance of a chunk 
...
We denote $\sigma$ as the percentage of unique data chunks belonging to PDS and 
...
It should be noted that the fingerprint-guided mapping used in ~\cite{extreme_binning09,Dong2011} narrows
the search scope of each data chunk, but it does not reduce th total amount of searchable fingerprints
used for the entire deduplication. In comparison we only focus on top popular chunks among VMs and this
significantly reduces the total amount of  searchable fingerprints. 

\item 
\textbf{VM-specific duplicate search optimization} --
While cross-VM deduplication is simplified, we intend to optimize the VM-sepcific deduplication 
as much as possible under a reasonable memory consumption
 to make up the loss of deduplication opportunites due to the cross-VM popularity constraint.
...
We define a parent  segment is  potentially similar to the currnet segment if 1) the paranet segment
