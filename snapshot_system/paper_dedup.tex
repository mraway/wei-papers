...
Our key observation is for those chunks that appear in different VMs, the top $k$ most popular items
...
into a file system block.  A  backend file system block can contain a large number of chunks.
In addition to the fact that a distributed file system block is often
configured to be large (e.g. 64MB), a number of chunks can be combined together
