...
our scheme achives a 85K:1 ratio between raw snapshot data and memory usage 
...
Each task handles the backup of one VM snapshop including deduplication.
...
When I/O throttling is not used, we will show later that processing multiple VMs concurrently 
...
In comparison, the sampled index method in~\cite{Guo2011} achives about 5.5GB per second
with 16 concurrent backup tasks and 6GB per second with
more tasks.  The per-machine throughput of our scheme is reasonable.
...
We demonstrate  the impact of adding extra replication for PDS in VC
and compare the snapshot availability of VC and VO. 
Appendix~\ref{sect:fault} provides a detailed analysis of 
snapshot availablility and the estimated result depends on how frequent a duplicate chunk
