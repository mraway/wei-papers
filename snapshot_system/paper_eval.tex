...
SRB executes distributed deduplication by routing a data chunks to cluster machines~\cite{Dong2011}
...
Column 2 of Table~\ref{fig:efficiency-comparison} shows deduplication efficiency of VC without 
...
significantly because of the controlled  disk read bandwidth usage.
...
It should be noted that global fingerprint  space requirement of VC is about 2\% of SRB 
(with $\sigma$=2\%) since we only deal with popular items. 
\comments{
The total memory space would cost over 6GB per machine.
We have implemented a bloomfilter for SRB which uses a comparable in-memory
...
\begin{table}[t]
  \centering
    ...
    \caption{Average time in microsecond to backup a dirty 2MB VM segment under SRB and VC with I/O throttling.
...
For our dataset with 1TB per machine to backup snapshots of 25 VMs, each machine
...
This experiment shows that the leakge of our approximate snapshot deletion is very small, 
