...
efficiency &          &       &           &      \\    \hline 
Memory(MB) &      46 & 118  & 108 & 2400 \\ \hline
    \end{tabular}
...
Memory usage per machine is low because each machine only hosts $1/p$  of  index for PDS plus some buffer space. 
...
the same offset in the parent segment to look for duplicates  but does not use 
the fingerprint-guided similarity search. 
...
The overall memory usage of VC for each physical machine is very small, which is not visible to other hosted cloud services.
The 2.4GB memory usage of SRB is very visible to other services even each physical machine has 32GB memory.
...
\subsection{More on Resource Usage and Processing Time}

We show  more experimental results on resource usage and processing time with SRB.
\comments{
\begin{table}
    \begin{tabular}{|c|c|}
    \hline
...
The ``Read'' column is controlled to 50MB/s bandwidth usage with I/O throttling so that other cloud services are not significantly impacted.
