...
the  average  processing  time of  a dirty VM segment in milliseconds
...
The space cost for deletion and leakage repair is very small.
Consider each reference consumes 8 bytes plus  1 mark bit. 
A VM that has 40GB backup data with about 10 million chunks will need less than 85MB of memory.
A VM-specific mark-and-sweep process takes  less than half an hour, when disk bandwidth is controlled as 50MB/s. 

When using the traditional mark-and-sweep~\cite{Guo2011,Fabiano2013} in VO for 2500 VMs, a total of 212GB memory
space on 100 node is accessed for processing each VM and each machine quires 2.12GB.
