...
Source-side deduplication (e.g. ~\cite{Symantic Netbean})
...
But it is memory-intensive to compare a large number of fingerprints and identify
...
resilience~\cite{Bhagwat2006}.  Separate files share the same physical copies of blocks that are
...
storage resource with them.  We propose a VM-centric approach that
simplifies deduplication and snapshot deletion.
The key benefit is that while maintaining  a competitive deduplication efficiency,
the scheme uses a small resource usage without affecting other collocated cloud services.
\comments{
