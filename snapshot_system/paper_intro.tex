...
separate computing and storage ``tiers.'')
...
%Google model. 
These providers use software to
...
share that block are affected.  The problem is particularly accute for VM
...
In this paper {\em we explore a method for deduplicating VM snapshots in
converged architecture IaaS systems that strikes a balance between fault
isolation and storage efficiency}.  The main contribution of this work is to
evaluate the impact of sharing only ``popular'' data blocks 
...
This work explores the storage and
...
converged IaaS cloud enviornments.
...
%deduplication is collocated with other services running on a shared compute
