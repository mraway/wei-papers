...
\begin{itemize}
\item \textbf{Local duplicate search with VM-specific bining.}
...
So when we are processing a dirty segment,
we can easily find similar segments from the
parent snapshot recipe and load their segment recipes. Then we can perform fingerprint matching 
against them
for further inner-VM deduplication.
